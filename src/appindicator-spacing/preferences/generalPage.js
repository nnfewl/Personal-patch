...
class AppIndicatorGeneralPage extends Adw.PreferencesPage {
    _init(settings, settingsKey) {
        ...
        const indicatorPaddingRow = this._createSpinRow({
            title: _('Horizontal Padding'),
            subtitle: _('Pixels on each side of every tray indicator'),
            settingsKey: this._settingsKey.INDICATOR_PADDING,
            from: 0,
            to: 24,
            step: 1,
            round: true,
        });
        indicatorPaddingRow.sensitive = compactModeSwitch.active;
        ...
        compactModeSwitch.connect('notify::active', widget => {
            ...
            indicatorPaddingRow.sensitive = widget.get_active();
        });
